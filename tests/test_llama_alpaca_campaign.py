"""Offline gates for a six-model, one-paid-invocation Alpaca campaign."""
import importlib
import json
...
def module():
    return importlib.import_module('scripts.llama_alpaca_campaign')
...
def campaign(tmp_path):
    ...
    files = set(GEN_SOURCES) | {'scripts/llama_alpaca_campaign.py',
        'scripts/llama_eval_worker.py', 'scripts/eval_policy.py', 'scripts/judge_alpaca.py'}
    for name in files:
        target = suite / 'source' / name; target.parent.mkdir(parents=True, exist_ok=True)
        if (ROOT / name).exists():
            shutil.copyfile(ROOT / name, target)
        else:
            target.write_text('# implementation pending\n')
    ...
    for tag in TAGS:
        ...
        bound = {}
        if tag != 'base':
            path = tmp_path / tag / ('sft' if tag == 'sft-init' else 'step-250')
    ...
    manifest = {'benchmark': 'alpaca805', 'base_model': str(base),
        ...
        'tokenizer': str(base), 'tokenizer_fingerprint': fingerprint(base, full_weights=False),
        'models': models, 'seeds': [42], 'expected_stop_token_ids': [128001, 128008, 128009],
        'actor_pad_token_id': 128004, 'prompt_token_ids_sha256': 'a' * 64,
    ...
    for tag in TAGS:
        ...
        rows = [{'idx': i, 'sample_idx': 0, 'instruction': ref['instruction'],
                 ...
                 'stop_reason': 128009, 'last_token_id': 128009} for i, ref in enumerate(refs)]
        ...
        cfg = {'protocol': 2, 'scorer': 'alpaca', 'recipe': {'temp': 1., 'n': 1, 'top_p': 1., 'top_k': -1},
            ...
            'stop_token_ids': manifest['expected_stop_token_ids'],
            'tokenizer': {'source': str(base), 'fingerprint': manifest['tokenizer_fingerprint']},
    ...
    return SimpleNamespace(suite=suite, manifest=manifest, refs=refs)
...
def test_commands_bind_fixed_sampling_and_one_shared_judge_budget(campaign):
    m = module(); command = m.generation_command(campaign.suite, campaign.manifest, 'base')
    assert command[command.index('--adapters') + 1] == 'base=none'
    ...
    assert command[command.index('--tags') + 1:command.index('--tags') + 7] == list(TAGS)
...
@pytest.mark.parametrize('damage', ['hash', 'missing_row', 'idx', 'sample', 'empty', 'metadata',
...
def test_generation_gate_rejects_corruption_even_when_rehashed(campaign, damage):
    ...
    elif damage == 'tokenizer': cfg['tokenizer']['source'] = 'wrong'
...
def test_immediate_eot_and_whitespace_responses_remain_in_all_805(campaign, text):
    directory = campaign.suite / 'generations/base'; path = directory / 'generations_t1.0_n1.jsonl'
    rows = [json.loads(line) for line in path.open()]
    rows[0].update(response=text, response_tokens=1)
    jsonl(path, rows)
    cache_path = directory / 'manifest_t1.0_n1.json'
    commit_cache(cache_path, json.loads(cache_path.read_text())['config'], [path])
    assert module().validate_generation(campaign.suite, 'base', campaign.manifest)['n'] == 805
...
def test_input_hash_failure_records_worker_failed_without_gpu_or_child(campaign, monkeypatch):
    ...
    monkeypatch.setattr(m, '__file__', str(campaign.suite / 'source/scripts/llama_alpaca_campaign.py'))
...
def test_watcher_waits_for_all_six_exit_codes(campaign, monkeypatch):
    m = module(); (campaign.suite / 'job/lam8/exit_code').unlink()
...
def test_watcher_persists_started_before_one_paid_call_and_verifies_all_results(campaign, monkeypatch):
    ...
    assert summary['n_annotations'] == 6 * 805 and summary['n_failed_parse'] == 6
