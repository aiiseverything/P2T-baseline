...
RED never needs the policy's full-vocabulary response logits for credit.  It
needs them only to re-derive the selected log-probs and the entropy, so every
reduction here runs in token blocks and the trainer keeps a per-response
microbatch.  That is a real saving over the VPO-RM arms, which must hold
``[B, T, V]`` at once to compute their softmax-over-vocabulary direction.
