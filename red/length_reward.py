"""The project's soft length window, mirrored so the RED arm is comparable.
...
These costs change the *sequence* reward, which is what the leave-one-out
baseline standardises.  They do not enter Eq. (6): the redistributed rewards are
built from the reward model's own prefix scores, and shaping them would break the
telescoping identity ``sum_t r~_t = R(x,y) - R(x, empty)``.  See
RED_REPRO_NOTES.md.
"""
...
def calibrate_reward_scale(rewards: Tensor, group_ids: Tensor, valid: Tensor) -> float:
    ...
    the paper never states: the RED diagnostics report it next to the baseline
    and the token rewards so the reader can see what scale the advantages live on.
    """
...
def guard_degenerate_rewards(rewards: Tensor, lengths: Tensor, group_ids: Tensor,
    ...
    cross-response coupled, which is exactly why Eq. (3) never sees it: R stays
    the reward model's own score for every response.
    """
