...
This matters more for RED than for the VPO-RM arms.  RED builds the
sampler-to-trainer correction ``importance = exp(old_logp - rollout_logprob)``
straight from the chosen-token log probability, so any head-precision difference
between the sampler and the trainer lands directly in it instead of cancelling
out.  Keeping both sides in FP32 removes the discrepancy rather than absorbing it.
