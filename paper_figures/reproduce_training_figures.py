...
def read_metrics(filename):
    with (INPUT / filename).open(newline='') as handle:
        rows = list(csv.DictReader(handle))
    return [{key: (int(value) if key == 'rollout' else float(value))
             if key not in ('cell', 'arm') else value
             for key, value in row.items() if value != ''} for row in rows]


def load_all():
    data = {}
    for row in read_metrics('rl_metrics_2x2.csv'):
        data.setdefault((row['cell'], row['arm']), []).append(row)
    assert len(data) == 8 and sum(map(len, data.values())) == 2000
    for rows in data.values():
        rows.sort(key=lambda row: row['rollout'])
        assert [r['rollout'] for r in rows] == list(range(1, 251))
    return data
...
def main():
    ...
    ablation = {}
    for row in read_metrics('rl_metrics_qwen_base_ablation.csv'):
        ablation.setdefault(row['arm'], []).append(row)
    assert len(ablation) == 5 and sum(map(len, ablation.values())) == 1250
...
if __name__ == '__main__':
    import argparse, os
    parser = argparse.ArgumentParser(description='Replot all training figures from the included CSV files; no original experiment paths are needed.')
    parser.add_argument('--out', type=Path, default=INPUT / 'reproduced')
