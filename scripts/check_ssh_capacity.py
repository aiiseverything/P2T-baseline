...
def prompt_lengths(actor_tokenizer, reward_tokenizer, prompt):
    """Measure the exact untruncated Actor and RM preparation paths."""
    ...
    reward_ids = VPOTrainer._render_chat_prompt(reward_tokenizer, prompt, tokenize=True)
    if hasattr(reward_ids, "input_ids"):
        reward_ids = reward_ids.input_ids
...
def build_synthetic_rollout(trainer, prompts):
    ...
    shape = {
        ...
        "reward_max_sequence_length": max(pair[1] for pair in lengths) + response_width,
