...
tokenizers, dataset files/schema, and the two-GPU resource budget.  Checks that
...
def check_model(ref: str, role: str, expected_pad: str, results: list[dict[str, Any]],
    ...
    if index_path.exists():
        ...
        absent = sorted({name for name in index.get("weight_map", {}).values()
                         if not (model_dir / name).exists()})
        if absent:
            _fail(f"{role}-weights", "missing safetensors shard(s): " + ", ".join(absent), results)
        else:
            size = sum((model_dir / n).stat().st_size for n in set(index.get("weight_map", {}).values()))
            _ok(f"{role}-weights", f"{len(set(index.get('weight_map', {}).values()))} shards, {size / 2**30:.2f} GiB", results)
    else:
        _warn(f"{role}-weights", "model.safetensors.index.json not found; weights not audited", results)
...
def check_tokenizers(actor: Path | None, rm: Path | None, pad_token: str,
    ...
    try:
        ...
        _warn("tokenizer-compatibility", f"could not load tokenizer ({type(exc).__name__}: {exc})", results)
...
def check_experiment_contract(cfg: dict[str, Any], setting: dict[str, Any] | None,
    ...
    if setting is None:
        return
    checks = {
        "hardware.gpu_count": (cfg.get("hardware", {}).get("gpu_count"), 2),
    ...
    if bad:
        ...
        _ok("experiment-contract", "Skywork two-GPU main-run parameters match experiments.json", results)
