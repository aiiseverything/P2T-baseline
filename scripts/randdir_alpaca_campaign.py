#!/usr/bin/env python3
"""AlpacaEval-805 campaign for the random-credit VPO lambda-4 ablation policy.

Adapted from the Llama-base and Qwen-instruct campaigns: one policy, one frozen
suite. `worker` runs inside a 1xH200 rjob (generation + full validation);
`watch` runs on a network-enabled host and dispatches the frozen GPT-4.1 judge
only after generation completed with exit code 0, then aggregates and verifies.
"""
...
from scripts.randdir_eval_worker import (require, save, now, read_jsonl, close_number,
    validate_adapter_result, validate_tokenizer_result, validate_prompt_protocol,
    validate_training_identity, TAGS, STOPS, PAD, VOCAB, RECIPE)
...
def validate_manifest(manifest):
    ...
    require(set(manifest['models']) == set(TAGS), 'Expected only the random-credit policy')
    require(manifest['tokenizer'] == manifest['base_model'] and Path(manifest['base_model']).is_absolute(),
            'The canonical evaluation renders with the absolute base model tokenizer')
    ...
    for tag, item in manifest['models'].items():
        require(item['adapter'] != 'none' and Path(item['adapter']).is_absolute(),
                'The random-credit policy must bind its checkpoint adapter')
...
def verify_inputs(suite, manifest, tag=None):
    ...
    required = {'source/' + name for name in GEN_SOURCES | {
        'scripts/randdir_alpaca_campaign.py', 'scripts/randdir_eval_worker.py',
...
def worker(suite, tag):
    ...
    with (job / 'worker.lock').open('a') as lock:
        ...
        try:
            require(Path(__file__).resolve() == suite / 'source/scripts/randdir_alpaca_campaign.py', 'Run the frozen worker')
    ...
    with (suite / 'judge.lock').open('a') as lock:
        ...
        try:
            ...
            save(suite / 'completion.json', {'status': 'complete', 'verified_at': now(),
                ...
                'files_sha256': {str(path.relative_to(suite)): file_hash(path) for path in artifacts}})
...
def main(argv=None):
    ...
    if args.command == 'worker':
        print(json.dumps(worker(args.suite, args.tag), default=str))
    else:
        while True:
