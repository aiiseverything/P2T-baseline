...
def eval_config(args, recipe, model_fingerprint, adapter_fingerprint, stop_ids,
    ...
    sources = ['scripts/eval_artifacts.py', f'scripts/eval_{scorer}.py',
               'vpo_rm/token_policy.py', 'vpo_rm/trainer.py',
...
def validate_adapter_base(adapter, model):
    ...
    if recorded_path.resolve() == Path(model).resolve():
        return
    # Relocated copies are valid when the architecture agrees. Incompatible
    # hidden dimensions (e.g. 8B versus 14B) fail before the engine allocates GPUs.
    if (recorded_path / 'config.json').is_file():
        a = json.loads((recorded_path / 'config.json').read_text())
        b = json.loads((Path(model) / 'config.json').read_text())
        fields = ('model_type', 'hidden_size', 'num_hidden_layers', 'num_attention_heads',
                  'num_key_value_heads', 'intermediate_size', 'vocab_size')
        if all(a.get(k) == b.get(k) for k in fields):
            return
    elif Path(recorded).name == Path(model).name:
        return
