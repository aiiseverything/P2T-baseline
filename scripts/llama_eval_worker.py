#!/usr/bin/env python3
"""Run and verify one frozen Llama policy evaluation on one H200.

Validation follows the existing reward256 and five-seed IFEval workers. Base/RM
weights use frozen filesystem fingerprints; adapter files use full SHA256.
No model weights are loaded by this worker's CPU preflight.
"""
...
def validate_manifest(manifest, tag):
    require(set(manifest['models']) == set(TAGS) and tag in TAGS, 'Expected exactly six Llama policies')
    ...
    require(manifest['expected_stop_token_ids'] == STOPS and manifest['actor_pad_token_id'] == 128004,
            'Llama stop/pad protocol changed')
    ...
    for label, item in manifest['models'].items():
        ...
        require((adapter == 'none') == (label == 'base'), 'Only Base may omit the adapter')
        if label not in ('base', 'sft-init'):
            require(Path(adapter).name in ('step-250', 'checkpoint-250'), 'RL evaluation requires final step 250')
        require(item['policy']['policy_head_dtype'] == 'float32', 'All six actor heads must use FP32')
...
def validate_reward_result(output, tag, manifest):
    ...
    for i, (row, generated) in enumerate(zip(rows, generations)):
        ...
        require(isinstance(ids, list) and all(type(t) is int and 0 <= t < 128256 for t in ids)
...
def verify_inputs(suite, manifest, tag):
    ...
    required = {'source/scripts/' + name for name in ('llama_eval_worker.py', entry, 'eval_artifacts.py', 'eval_policy.py')}
...
def validate_prompt_protocol(tokenizer, prompts, manifest):
    ...
    require(tokenizer.bos_token_id == 128000 and tokenizer.pad_token_id == 128004
            and tokenizer.eos_token_id == 128009 and list(get_stop_token_ids(tokenizer)) == STOPS,
            'Actor tokenizer native BOS/pad/stop protocol changed')
    ...
    require(all(row and row[0] == 128000 and row.count(128000) == 1 for row in rows), 'Expected exactly one native BOS')
...
def main(argv=None):
    ...
    with (job / 'worker.lock').open('a') as lock:
        ...
        try:
            require(Path(__file__).resolve() == suite / 'source/scripts/llama_eval_worker.py',
