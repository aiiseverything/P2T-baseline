#!/usr/bin/env python3
"""Frozen four-policy Llama-base Alpaca generation and one bounded GPT-4.1 judge invocation.

Policies: bare Llama-3.1-8B (`base`), its native-EOS SFT initialization (`sft`),
and the GRPO / VPO lambda-4 checkpoints trained from it (`grpo`, `lam4`). All four
render with the saved SFT tokenizer because the base checkpoint has no chat template.
"""
...
from scripts.llama_base_eval_worker import (require, save, now, read_jsonl, close_number,
    validate_adapter_result, validate_tokenizer_result, validate_prompt_protocol,
    validate_training_identity, TAGS, STOPS, PAD, EOS, VOCAB, STEP0, ADAPTER_FREE,
...
def validate_manifest(manifest):
    ...
    require(set(manifest['models']) == set(TAGS), 'Expected base, sft, grpo and lam4 policies')
    sft = manifest['models']['sft']['adapter']
    require(sft != 'none' and manifest['tokenizer'] == sft and Path(sft).is_absolute()
            and Path(manifest['base_model']).is_absolute(),
            'Every policy must render with the absolute saved SFT tokenizer')
    require(manifest['expected_stop_token_ids'] == STOPS and manifest['actor_pad_token_id'] == PAD
            and manifest['actor_response_eos_id'] == EOS
            and manifest['chat_template_sha256'] == CHAT_TEMPLATE_SHA256, 'Actor stop/pad/EOS/template protocol changed')
    ...
    for tag, item in manifest['models'].items():
        require((item['adapter'] == 'none') == (tag in ADAPTER_FREE), 'Only the bare base may omit its adapter')
...
def verify_inputs(suite, manifest, tag=None):
    ...
    required = {'source/' + name for name in GEN_SOURCES | {
        'scripts/llama_base_alpaca_campaign.py', 'scripts/llama_base_eval_worker.py',
    ...
    for label in ([tag] if tag is not None else TAGS):
        ...
        validate_training_identity(manifest, label)
    references(suite)
...
def generation_command(suite, manifest, tag):
    ...
    return [sys.executable, str(suite / 'source/scripts/eval_alpaca.py'),
        '--model', manifest['base_model'], '--tokenizer', manifest['tokenizer'],
        '--dataset', str(suite / 'references.jsonl'),
...
def validate_generation(suite, tag, manifest):
    ...
    for i, (row, ref) in enumerate(zip(rows, refs)):
        ...
        require(type(row['response_tokens']) is int and 0 < row['response_tokens'] <= 2048
                ...
                and type(row['last_token_id']) is int and 0 <= row['last_token_id'] < VOCAB
...
def worker(suite, tag):
    ...
    with (job / 'worker.lock').open('a') as lock:
        ...
        try:
            require(Path(__file__).resolve() == suite / 'source/scripts/llama_base_alpaca_campaign.py', 'Run the frozen worker')
    ...
    with (suite / 'judge.lock').open('a') as lock:
        ...
        try:
            ...
            summary = {'status': 'complete', 'verified_at': now(), 'experiment_sha256': experiment_sha,
                ...
                'n_annotations': len(TAGS) * 805, 'n_failed_parse': sum(row['n_failed_parse'] for row in results),
