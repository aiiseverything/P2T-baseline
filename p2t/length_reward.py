"""The project's soft length window, mirrored so the P2T arm is comparable.
...
These costs change the *sequence* reward that Eq. (4) standardises.  They do not
enter Eq. (3): the paper's R is the reward model's score, and a sign-flipping
shaped reward would invert the meaning of the omega term.  See
P2T_REPRO_NOTES.md.
"""
...
def calibrate_reward_scale(rewards: Tensor, group_ids: Tensor, valid: Tensor) -> float:
    ...
    the paper never states: the P2T diagnostics report it next to alpha*R so the
    reader can see how far the actor's reward model is from the O(1) scale
    alpha = 0.1 presumes.
    """
