...
* Special tokens are decoded away by ``skip_special_tokens=True``, so they are
  ...
  of the canonical chat, which *is* a special token -- so the attribution at the
  exact position the reward model reads is structurally zero.  That is a
  property of applying the paper to a chat-template reward model, not a bug, and
  the trainer logs ``p2t_unmapped_share_mean`` so it stays visible.
* Tokens that a BPE merge/split rewrites receive -1.  Attribution is never
  invented for them; they keep the sequence-level part of Eq. (5) only.
"""
