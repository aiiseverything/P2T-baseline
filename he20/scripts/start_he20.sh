#!/usr/bin/env bash
# Launch a he20 run detached, so it survives a closed terminal.
#
#   bash he20/scripts/start_he20.sh he20250 [extra run_he20.py args...]
#
# A self-contained copy of the sibling arm's launcher on purpose: nothing here
# touches the shared scripts/ directory, so a he20 run can never be blocked by, or
# accidentally disturb, a P2T run that is already using those files.
set -euo pipefail

RUN="${1:?usage: start_he20.sh <run-name> [args...]}"
shift || true
...
# used:
#
#   HE20_GPUS=4,5,6   --1,2   remaps physical cards to 0,1,2, so the config must
...
#   HE20_GPUS unset    --     nothing is remapped, so the config names absolute
...
# actor/reward entries are indices into HE20_GPUS, without it they are absolute.
...
{
  echo "=== he20 run $RUN started $(date --iso-8601=seconds) on GPUs $GPUS ==="
} >> "$LOG_FILE"

setsid nohup "$ROOT/.venv/bin/python" he20/scripts/run_he20.py --config "configs/$RUN.json" "$@" \
  >> "$LOG_FILE" 2>&1 &
echo $! > "$PID_FILE"
