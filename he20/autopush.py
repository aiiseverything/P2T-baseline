...
class AutoPusher:
    def __init__(self, *, enabled: bool, every: int, remote: str, branch: str,
        ...
        # always the same name: the sibling P2T arm lives on a local
        # ``p2t-baseline`` while its dedicated remote takes it as ``main``.
        # Conflating the two made every push abort with "refused_wrong_branch",
        # so no step ever pushed.
        self.branch = branch
    ...
    def _push_locked(self, step: int, metrics: dict) -> bool:
        ...
        message = (f"he20 step {step}: {summary}\n\n"
