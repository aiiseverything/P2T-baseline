...
The reward model never sees the actor's token IDs directly -- it sees its own
rendering of the decoded text -- so the two sides only agree on a response token
if its bytes survive the round trip unchanged.  This module builds the canonical
score input and finds those correspondences.

This is a port of ``vpo_rm/reward_inputs.py`` from the parent project, kept
behaviourally identical so every arm scores the same text the same way: a token
that cannot be mapped gets position -1.
...
* Special tokens are decoded away by ``skip_special_tokens=True``, so they are
  ...
  of the canonical chat, which *is* a special token -- so the last reward-model
  token sits past the last mapped response token.  The trainer logs
  ``he20_unmapped_share_mean`` so the unmapped share stays visible.
* Tokens that a BPE merge/split rewrites receive -1.  No reward-model position is
  ever invented for them; they simply contribute nothing.
"""
