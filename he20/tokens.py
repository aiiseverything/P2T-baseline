...
The actor and the reward model must share one token-to-ID mapping: the reward
model is scored on its own rendering of the actor's decoded response, so the two
sides only agree on a token if its ID means the same thing on both.  The parent
project's ``vpo_rm.alignment`` enforces that; we re-assert it here so the
standalone package cannot silently run with mismatched tokenizers.
"""
...
def load_actor_tokenizer(model_name, init_adapter="", tokenizer_name="", *,
    ...
    that the shared actor and reward-model protocol depends on.
    """
    import hashlib
    import json
    from pathlib import Path
