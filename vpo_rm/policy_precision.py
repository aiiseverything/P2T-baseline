"""FP32 policy output projection with ordinary autograd for hidden states."""
...
class FrozenFP32OutputHead(nn.Linear):
    """A frozen Linear preserving ``weight``/``bias`` keys and hidden autograd.

    Casting the hidden states is differentiable. The FP32 projection does not
    round its output back to the decoder dtype, including under autocast.
    """
...
def enable_fp32_output_head(actor) -> FrozenFP32OutputHead:
    """Install an untied, frozen FP32 head; call again after adapter reload."""
