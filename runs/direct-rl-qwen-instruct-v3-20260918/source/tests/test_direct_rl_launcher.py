...
@pytest.mark.parametrize('family', ['qwen'])
def test_fresh_config_preserves_canonical_algorithm_and_final_only(family):
    ...
    assert changed == {'model', 'init_adapter', 'checkpoint_interval'}
    assert new['model'].endswith('/models/Qwen3-14B')
...
def test_fresh_config_rejects_any_nonempty_initialization(bad):
    m = launcher(); c = m.build_config('qwen'); c['init_adapter'] = bad
