#!/usr/bin/env python3
"""Prepare the corrected posttrained Qwen-only fresh-LoRA, final-only RL suite.
...
QWEN_MODEL = SHARED / 'models/Qwen3-14B'
QWEN_REPO = 'Qwen/Qwen3-14B'
QWEN_REVISION = '40c069824f4251a91eefaf281ebe4c544efd3e18'
...
def build_config(family):
    ...
    config.update(model=str(QWEN_MODEL), init_adapter='', checkpoint_interval=config['max_rollouts'] + 1)
    validate_config(config)
    return config


def validate_config(config):
    if Path(config.get('model', '')).resolve() != QWEN_MODEL.resolve():
        raise ValueError('This replacement suite requires the verified posttrained Qwen actor')
...
def _download_record(config):
    root = Path(config['model']).resolve()
    path = root / 'DOWNLOAD_MANIFEST.json'
    record = json.loads(path.read_text())
    if (root != QWEN_MODEL.resolve() or record.get('verified') is not True
            or record.get('upstream_repo') != QWEN_REPO
            or record.get('upstream_revision') != QWEN_REVISION
            or not isinstance(record.get('destination'), str)
            or Path(record['destination']).resolve() != root):
        raise ValueError('Actor download must verify the pinned official posttrained Qwen repository')
    return root, path, record


def verify_actor_download(config):
    """Hash every downloaded file once before preparing a new immutable suite."""
    root, path, record = _download_record(config)
    manifest_sha = file_hash(path)
    rows = record.get('files')
    if not isinstance(rows, list) or not rows:
        raise ValueError('Actor download file evidence is missing')
    required = {'config.json', 'generation_config.json', 'tokenizer.json', 'tokenizer_config.json',
                'merges.txt', 'vocab.json', 'model.safetensors.index.json', 'LICENSE'}
    weights = {f'model-{index:05d}-of-00008.safetensors' for index in range(1, 9)}
    names = [row.get('path') for row in rows if isinstance(row, dict)]
    if len(names) != len(rows) or len(set(names)) != len(names) or set(names) != required | weights:
        raise ValueError('Actor download must contain the exact required files and eight weight shards')
    audited = []
    for row in rows:
        name = row['path']
        current = root / name
        if (Path(name).is_absolute() or '..' in Path(name).parts or current.is_symlink()
                or current.resolve().parent != root or not current.is_file()):
            raise ValueError('Actor download file path escapes its verified directory')
        before = current.stat()
        identity = {'size': before.st_size, 'mtime_ns': before.st_mtime_ns,
                    'ctime_ns': before.st_ctime_ns, 'inode': before.st_ino}
        expected = row.get('local_sha256')
        if (row.get('verified_against_upstream') is not True or type(row.get('size')) is not int
                or row['size'] != before.st_size or not isinstance(expected, str) or len(expected) != 64
                or row.get('source_sha256') != expected or file_hash(current) != expected):
            raise ValueError(f'Actor file SHA256/size verification failed: {name}')
        kind, upstream = row.get('upstream_identity_kind'), row.get('upstream_identity')
        if kind == 'sha256':
            upstream_matches = upstream == expected
        elif kind == 'git_blob_sha1' and name not in weights:
            data = current.read_bytes()
            upstream_matches = upstream == hashlib.sha1(b'blob ' + str(len(data)).encode() + b'\0' + data).hexdigest()
        else:
            upstream_matches = False
        after = current.stat()
        if (not upstream_matches or identity != {'size': after.st_size, 'mtime_ns': after.st_mtime_ns,
                'ctime_ns': after.st_ctime_ns, 'inode': after.st_ino}):
            raise ValueError(f'Actor upstream identity/file stability verification failed: {name}')
        audited.append({'path': name, 'sha256': expected, **identity})
    architecture = json.loads((root / 'config.json').read_text())
    if architecture.get('architectures') != ['Qwen3ForCausalLM'] or architecture.get('model_type') != 'qwen3':
        raise ValueError('Actor architecture differs from posttrained Qwen3ForCausalLM')
    index = json.loads((root / 'model.safetensors.index.json').read_text())
    if (not isinstance(index.get('weight_map'), dict) or not index['weight_map']
            or set(index['weight_map'].values()) != weights
            or {p.name for p in root.glob('*.safetensors')} != weights):
        raise ValueError('Actor index does not bind exactly the eight verified weight shards')
    if file_hash(path) != manifest_sha:
        raise ValueError('Actor download manifest changed during full file hashing')
    return {'manifest': str(path), 'manifest_sha256': manifest_sha, 'upstream_repo': QWEN_REPO,
            'upstream_revision': QWEN_REVISION, 'weight_shards': 8, 'files': audited,
            'verification': 'full SHA256/upstream identity audit before preparation'}


def validate_actor_download_evidence(config, evidence):
    """Bind the full pre-prepare audit to unchanged file identities at runtime."""
    root, path, _ = _download_record(config)
    if (evidence.get('manifest') != str(path) or evidence.get('manifest_sha256') != file_hash(path)
            or evidence.get('upstream_repo') != QWEN_REPO
            or evidence.get('upstream_revision') != QWEN_REVISION or evidence.get('weight_shards') != 8
            or not evidence.get('files')):
        raise ValueError('Frozen actor download evidence changed')
    for row in evidence['files']:
        current = root / row['path']
        if Path(row['path']).is_absolute() or '..' in Path(row['path']).parts or current.is_symlink():
            raise ValueError('Invalid frozen actor audit path')
        stat = current.stat()
        ...
        if any(row.get(key) != value for key, value in actual.items()):
            raise ValueError('Actor file changed since full hashing: ' + row['path'])
    return evidence


def verify_input_assets(config, baseline):
    download = verify_actor_download(config)
    assets = {role: fingerprint(config[key], full_weights=False)
              for role, key in (('actor', 'model'), ('reward', 'rm'))}
    if assets['reward'] != baseline['validation_identity']['input_fingerprints']['reward']:
        raise ValueError('The Qwen reward model differs from the successfully trained canonical suite')
    validate_actor_download_evidence(config, download)
    return {'input_fingerprints': assets, 'actor_download': download}
...
def validation_identity(project_root):
    ...
    actor_download = validate_actor_download_evidence(config, manifest['actor_download'])
    ...
    return {'experiment_family': manifest['family'],
            ...
            'config': config, 'input_fingerprints': current, 'actor_download': actor_download,
...
def prepare(suite, outputs):
    ...
    if shutil.disk_usage('/data').free < 40 * 2**30:
        raise RuntimeError('Need 40 GiB free for final checkpoints, transient files and concurrent experiments')
...
'''
    ...
    for family in BASELINES:
        ...
        asset_evidence = verify_input_assets(config, baseline)
        assets = asset_evidence['input_fingerprints']
        ...
        manifest = {'created_at': datetime.now(timezone.utc).isoformat(), 'family': family,
            ...
            'actor_download': asset_evidence['actor_download'],
            'actor_variant': 'official_posttrained',
    ...
    write_json(suite / 'submission-plan.json', {'status': 'prepared_not_submitted',
        'families': manifests, 'gpu_per_job': 3, 'max_jobs': 2,
